...
- Pro-signed requestit lisäävät `user`, `nonce`, `signer` ja `signature` -kentät ja allekirjoittavat täsmälleen saman form-urlencoded-parametrimerkkijonon EIP-712 `AsterSignTransaction` -viestinä. `user` on päätilin wallet, johon signer-agentti on liitetty; ilman sitä Aster voi palauttaa `No agent found`.
...
4. Syötä `Signer wallet` eli tähän päätiliin Asterissä liitetty Pro API / agent signer -osoite.
